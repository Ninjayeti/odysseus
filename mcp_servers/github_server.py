...
env var that the spawner injects when the server is started. For now
we run one MCP instance per app process, scoped to whichever owner
...
# ── PAT resolution ──
# We load once per process and cache. If the user updates the PAT in
# settings, the MCP server is restarted by the integration routes so a
# stale cache is impossible in normal flow.
...
def _trim_notification(n: dict) -> dict:
    ...
    return {
        ...
        # subject.url is an API URL; convert to a usable html link by hitting the URL
        # later if the agent wants details. Keep the API URL so tools can chain.
...
TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        description=(
            ...
            "starting point when the user asks 'what's new' or 'anything need my attention'."
        ),
        inputSchema={
            "type": "object",
            "properties": {
