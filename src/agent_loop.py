...
def _build_system_prompt(
    ...
    # thousands of tokens back — models weight recent instructions more heavily,
    # so their own baked-in conventions override the briefing by position.
    # Re-injecting it as a protected system message right before the last user
    # message ensures the mechanical rules (commit trailers, PR format, force-
    # push policy) are fresh in context at the moment of action. Same pattern
    # as the document injection above, same reason.
    if github_briefing:
        _brief_msg = {
            "role": "system",
            "content": (
